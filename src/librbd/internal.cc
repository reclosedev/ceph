...
reprotect_and_return_err:
  ...
  void close_image(ImageCtx *ictx)
  {
    ...
    while (ictx->pending_readaheads > 0) {
      ictx->pending_readaheads_cond.Wait(ictx->readahead_lock);
    }
	       ...
	       char *buf, bufferlist *pbl, AioCompletion *c)
  {
	...
	for (map<object_t,vector<ObjectExtent> >::iterator p = readahead_object_extents.begin(); p != readahead_object_extents.end(); ++p) {
	  for (vector<ObjectExtent>::iterator q = p->second.begin(); q != p->second.end(); ++q) {
		...
		ictx->readahead_lock.Lock();
		ictx->pending_readaheads--;
		if (ictx->pending_readaheads == 0) {
		  ictx->pending_readaheads_cond.Signal();
		}
		ictx->readahead_lock.Unlock();
	      }
	    };
